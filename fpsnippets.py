...
zip_poly = ZIPS[ZIPS["ZIP"] == "15206"].geometry.iloc[0]
print(zip_poly)

# Check intersections with all neighborhoods
for _, hood in NEIGHBORHOODS.iterrows():
    if hood.geometry.intersects(zip_poly):
        print(hood["hood"])
...
redlined = geopandas.read_file(r'\Users\kilom\Desktop\InfoVis\final project possible attempt 1\geojson.json')

def geo_to_redlined(latitude, longitude):

    # Create a shapely point for the latitude and longitude
    pt = Point(longitude, latitude)
    # Loop through the neighborhoods
    matches = redlined[redlined.geometry.contains(pt)]
    # Check if this neighborhood contains the point
    if not matches.empty:
        # Return the name of the neighborhood
       return matches.iloc[0]["grade"]
    # Wasn't contained in the neighborhood
    return None
